...
import com.astrolink.dao.CtccSendSmsDataLinstenerThreadDao;
import com.astrolink.dao.CtccSendSmsExecLinstenerThreadDao;
 ...
 * @date 2015-11-30
 */
public class CtccSendSmsService implements IDbObjectReciver,IExecFinish {
	private static CtccSendSmsService instance;
	private static Logger logger = Logger.getLogger(CtccSendSmsService.class);
	//缓冲任务池
	...
	public static CtccSendSmsService getInstance(){
		if (instance == null) {
			instance = new CtccSendSmsService();
		}
		return instance;
	}
	
	public void doCtccSendSmsExec(){
		logger.info("启动电信包月短信下发监听服务");
		
		CtccSendSmsDataLinstenerThreadDao dlt = new CtccSendSmsDataLinstenerThreadDao();
	...
	public synchronized void dataRecived(Object object) {
		...
		while (iterator.hasNext()) {
			...
			if (!execMap.containsKey(entry.getKey())) {
				logger.info("查询到,电信电信包月短信下发处理线程，开始添加，id为" + entry.getKey());
				execMap.put(entry.getKey(), entry.getValue());
				// 添加服务器访问线程
				entry.getValue().getId();
				CtccSendSmsExecLinstenerThreadDao cslt=new CtccSendSmsExecLinstenerThreadDao(entry.getValue());
