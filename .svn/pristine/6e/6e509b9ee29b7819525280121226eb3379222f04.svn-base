...
public class NetManager {
	...
	public static NetManager getInstance() {
		if (instance == null) {
			 ...
			 * instance.sslContextForTrustedCertificates(); mSSLSocketFactory =
	...
	public SSLContext sslContextForTrustedCertificates() {
		 ...
		 * javax.net.ssl.TrustManager[1]; javax.net.ssl.TrustManager tm = new
		 * miMT(); trustAllCerts[0] = tm; SSLContext sc = null; try { sc =
		 * SSLContext.getInstance("SSL"); sc.init(null, trustAllCerts, null); //
		 * javax.net.ssl.HttpsURLConnection.setDefaultSSLSocketFactory(sc.
		 * getSocketFactory()); } catch (NoSuchAlgorithmException e) {
	...
	public NetResult HTTPSConnectPost(String urlString, String[][] headers, String[][] upload) {
		NetResult result = new NetResult();
		if (!urlString.startsWith("https://")) {
			urlString = "https://" + urlString;
		}
	...
	// HTTP连接
	public NetResult HTTPConnectGet(String urlString, String[][] headers, boolean auto) {
		NetResult result = new NetResult();
	...
	public final NetResult HTTPConnectPost(String url, String[][] headers, String[][] upload) {
		return HTTPConnectPost(url, headers, upload, null);
	}

	public final NetResult HTTPConnectPost(String url, String[][] headers, String[][] upload, String xml) {
		NetResult result = new NetResult();
		...
		try {
			...
			if (upload != null && upload.length > 0) {
				...
				for (int i = 0; i < upload.length; i++) {
					BasicNameValuePair pair = new BasicNameValuePair(upload[i][0], upload[i][1]);
					uploadPair.add(pair);
				}
				UrlEncodedFormEntity entity = new UrlEncodedFormEntity(uploadPair, HTTP.UTF_8);
	...
	public String getCookie(URLConnection conn) {
		...
		for (Entry<String, List<String>> entry : entrySet) {
			...
			for (String string : value) {
				if (key != null && !"".equals(key) && string != null && !"".equals(string)) {
					sb.append(key + "=");
					sb.append(string + ";");
				}
			}
	 ...
	 * 读取指定字节的内容 leng 字节长度
	 */
	public static byte[] readInstream(InputStream inputStream, int leng) throws Exception {
