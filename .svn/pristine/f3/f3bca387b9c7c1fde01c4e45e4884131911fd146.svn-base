 ...
 * @date 2016-09-08
 */
public class VideoDataLinstenerThreadDao2 extends JobThread{
	private Logger logger = Logger.getLogger(VideoDataLinstenerThreadDao2.class);
			...
			try {
				...
				String sql="SELECT a.id,a.ip,a.proxyIp,a.mobile,a.imei,a.imsi,a.iccid,a.cpparam,a.provId,a.cpInfoId,a.ssid,a.infoState,a.supportState,a.supportType,a.pramType,a.methodType,a.responseType,a.execNum,a.dayTime,a.inputDate,a.updateDate,a.execStep,a.execResultCode,a.execResultInfo,a.execState,e.consumeCode,s.supportAttrInfo,d.crackSynUrl  " +
						...
						"where infoState=0 AND supportState<1000001" + " " +
							...
							"AND supportType='video.VideoLogic'"+
							"AND execState = 0 "+
				            "AND ((execNum=0 AND (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(a.updateDate))>= 5) " +
							"OR (execNum=1 AND (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(a.updateDate))>= 10) " +
						    "OR (execNum>=2 AND (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(a.updateDate))>= 15)) order by execNum,a.id	limit "+(200-taskNum);
				ArrayList<Object> list=dt.executeQuery(sql, SupportExecLog.class, VideoDataLinstenerThreadDao2.class);
				taskNum=list.size();
