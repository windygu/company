...
public class ComplaintService implements IDbObjectReciver, IExecFinish {

	private static ComplaintService instance;
	private static Logger logger = Logger.getLogger(ComplaintService.class);
	//缓冲任务池
	private ConcurrentHashMap<Object, Complaint> waitMap = new ConcurrentHashMap<Object, Complaint>();
	//执行任务池
	...
	public void doComplaintExec() {
		logger.info("数据同步监听服务");
		
		ComplaintLinstenerThreadDao blt = new ComplaintLinstenerThreadDao();
	...
	public synchronized void dataRecived(Object object) {
		...
		for(Object o:list){
			...
			if(waitMap.size()<WAIT_MAP_MAX && !waitMap.containsKey(complaint.getSpComplaintId()) && !execMap.containsKey(complaint.getSpComplaintId())) {
				waitMap.put(complaint.getSpComplaintId(), complaint);
			}
		...
		Iterator<Entry<Object, Complaint>> iterator = waitMap.entrySet().iterator();
		while (iterator.hasNext()) {
			Entry<Object, Complaint> entry = iterator.next();
			
			if (!execMap.containsKey(entry.getKey())) {
				logger.info("查询到,数据同步处理线程，开始添加，id为" + entry.getKey());
				execMap.put(entry.getKey(), entry.getValue());
				//添加服务器访问线程
				ComplaintExecLinstenerThreadDao cslt = new ComplaintExecLinstenerThreadDao(entry.getValue());
