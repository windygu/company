...
import com.astrolink.dao.SynDataExecLinstenerThreadDao;
import com.astrolink.dao.SynDataLinstenerThreadDao;
 ...
 * @date 2015-11-30
 */
public class SynDataService implements IDbObjectReciver,IExecFinish {
	private static SynDataService instance;
	private static Logger logger = Logger.getLogger(SynDataService.class);
	//缓冲任务池
	...
	public void doSynDataExec(){
		...
		SynDataLinstenerThreadDao dlt = new SynDataLinstenerThreadDao();
	...
	public synchronized void dataRecived(Object object) {
		...
		while (iterator.hasNext()) {
			...
			if (!execMap.containsKey(entry.getKey())) {
				...
				// 添加服务器访问线程
				SynDataExecLinstenerThreadDao cslt=new SynDataExecLinstenerThreadDao(entry.getValue());
