...
import com.astrolink.dao.MmDataLinstenerThreadDao;
import com.astrolink.dao.MmExecLinstenerThreadDao;
 ...
 * @date 2015-11-30
 */
public class MmService implements IDbObjectReciver,IExecFinish {
	private static MmService instance;
	private static Logger logger = Logger.getLogger(MmService.class);
	//缓冲任务池
	private ConcurrentHashMap<Object, MmExecLog> waitMap = new ConcurrentHashMap<Object, MmExecLog>();
	//执行任务池
	...
	public static MmService getInstance(){
		if (instance == null) {
			instance = new MmService();
		}
		return instance;
	}
	
	public void doMmExec(){
		logger.info("启动动漫计费确认监听服务");
		
		MmDataLinstenerThreadDao dlt = new MmDataLinstenerThreadDao();
		dlt.setDbObjectReciver(this);
		dlt.setDbTableName(dbTableName);
		ThreadPoolManager.getInstance().addJobThread("MmExecLog", dlt);
	...
	public synchronized void dataRecived(Object object) {
		...
		for(Object o:list){
			MmExecLog mmExecLog=(MmExecLog)o;
			if(waitMap.size()<WAIT_MAP_MAX && !waitMap.containsKey(mmExecLog.getId()) && !execMap.containsKey(mmExecLog.getId())){
				waitMap.put(mmExecLog.getId(), mmExecLog);
			}
		...
		Iterator<Entry<Object, MmExecLog>> iterator = waitMap.entrySet().iterator();
		while (iterator.hasNext()) {

			Entry<Object, MmExecLog> entry = iterator.next();
			...
			if (!execMap.containsKey(entry.getKey())) {
				...
				MmExecLinstenerThreadDao cslt=new MmExecLinstenerThreadDao(entry.getValue());
