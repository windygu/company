...
import com.astrolink.dao.JdGgExecLinstenerThreadDao;
import com.astrolink.dao.JdGgLinstenerThreadDao;
import com.astrolink.domain.DmLogin;
import com.astrolink.system.pool.ThreadPoolManager;

public class JdGgService implements IDbObjectReciver, IExecFinish {

	private static JdGgService instance;
	private static Logger logger = Logger.getLogger(JdGgService.class);
	//缓冲任务池
	...
	public static JdGgService getInstance() {
		if(instance == null) {
			instance = new JdGgService();
		}
		return instance;
	}
	
	public void doJdGgExec() {
		logger.info("数据导入监听服务");
		
		JdGgLinstenerThreadDao blt = new JdGgLinstenerThreadDao();
	...
	public synchronized void dataRecived(Object object) {
		...
		while (iterator.hasNext()) {
			...
			if (!execMap.containsKey(entry.getKey())) {
				...
				//添加服务器访问线程
				JdGgExecLinstenerThreadDao cslt = new JdGgExecLinstenerThreadDao(entry.getValue());
