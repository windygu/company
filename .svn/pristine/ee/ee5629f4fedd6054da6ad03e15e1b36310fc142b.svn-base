	...
	public String getResult(String codeID) {
		...
		while (result.equals( "-3") && timer < timeOut) {
			...
			try {
				...
				System.out.println("轮询服务器"+i+"返回：  "+result);
				Thread.sleep(1*1000);
			} catch (Exception e) {
			} finally{
				if(i>=20){
	...
	public String autoRecognition(String imagePath, String codeType) {
		...
		System.out.println("上传返回："+result);
			...
			if (arrayResult.length==2) {
				System.out.println("识别结果是："+arrayResult[1]);
				...
				if (!postData.isEmpty() && postData.size() > 0) {
					...
					for (Entry<String, String> ent : ((Map<String, String>) postData).entrySet()) {
						if (ent.getKey().equals("img")){
							builder.addBinaryBody(ent.getKey(), new File(ent.getValue()));
						}else {
							builder.addTextBody(ent.getKey(), ent.getValue());
						}
					}
